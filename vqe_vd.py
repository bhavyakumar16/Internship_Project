...
def vqe_ops(theta, qubits):
    def build(qc):
        qc.ry(theta[0], qubits[0])
        qc.ry(theta[1], qubits[1])
        qc.cx(qubits[0], qubits[1])
        qc.ry(theta[2], qubits[0])
        qc.ry(theta[3], qubits[1])
    return build

def full_circuit():
    qc = QuantumCircuit(2, 2)
    vqe_ops(theta, [0, 1])(qc)
    qc.measure([0, 1], [0, 1])
    return qc

# -----------------------------
# Joint ZZ parity (matches Bell/GHZ VD methodology - a joint, symmetric
# observable compatible with VD's derangement-circuit formalism, rather
# than an isolated single-qubit Z0)
# -----------------------------
...
vqe_ops(theta, [0, 1])(vd_qc)
vqe_ops(theta, [2, 3])(vd_qc)
...
plt.title("VQE Ansatz: Ideal vs Noisy vs VD-Mitigated")
plt.axhline(y=ideal_val, color='gray', linestyle='--', linewidth=1)
plt.savefig("results/vqe_vd_comparison.png", dpi=300, bbox_inches="tight")
plt.close()

print("\nSaved chart to results/vqe_vd_comparison.png")